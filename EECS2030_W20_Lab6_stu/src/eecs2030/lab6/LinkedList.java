...
public class LinkedList
{
	...
	public int numOfTimes( int k )
	{
		...
		return numOfTimesH(k,head);
   		//return counter;	// replace this line with your code
	}  // end numOfTimes
	
	public int numOfTimesH(int k, Node node) {
		int counter;
		if(node.next==null) {
			counter= 0; //not found till end
		}
		
		else {
			if(node.data==k) {//found i++, counter++
				counter= 1 + numOfTimesH(k,node.next);
			}
			else {//i++, index i
				counter= 0+ numOfTimesH(k,node.next);
			}
		}
